...
describe('application shell', () => { it('renders Today and all bottom navigation destinations', async () => { await renderRouter({ '(tabs)/_layout': TabLayoutWithTheme, '(tabs)/index': TodayWithApplication, '(tabs)/projects': ProjectsScreen, '(tabs)/tasks': TasksScreen, '(tabs)/planning': PlanningScreen, '(tabs)/more': MoreScreen }, { initialUrl: '/' }); expect(screen.getByText('Active projects')).toBeTruthy(); expect(screen.getAllByText('Today').length).toBeGreaterThan(0); expect(screen.getByText('Projects')).toBeTruthy(); expect(screen.getByText('Tasks')).toBeTruthy(); expect(screen.getByText('Planning')).toBeTruthy(); expect(screen.getByText('More')).toBeTruthy(); }, 10_000); });